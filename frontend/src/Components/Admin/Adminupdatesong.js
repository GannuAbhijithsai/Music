...
export default function Adminupdatesong() {
...
const updatenav=useNavigate();
  const editsong=async()=>{
    ...
    console.log(name);
      try {
          const response = await fetch(`${process.env.REACT_APP_BASE_URL}/api/songs/update/${localStorage.getItem("updatesongid")}`, {
            method: 'PUT',
            headers: {
            ...
            body :JSON.stringify({
              "name":name===""?localStorage.getItem("updatesongname"):name,
              "artist":artist===""?localStorage.getItem("updatesongartist"):artist,
              "song":song===undefined?localStorage.getItem("updatesongsong"):song,
              "img":image===undefined?localStorage.getItem("updatesongimage"):image,
              "duration":time===undefined?localStorage.getItem("updatesongduration"):time,
     })
          });
         ...
         if(response.status!==200){
          ...
          alert("Song has been updated Successfully");
          localStorage.removeItem("updatesongid");
          localStorage.removeItem("updatesongname");
          localStorage.removeItem("updatesongartist");
          localStorage.removeItem("updatesongsong");
          localStorage.removeItem("updatesongimage");
          localStorage.removeItem("updatesongduration");
          updatenav("/admin/allsongs");
         }
        } catch (error) {
          setalt(true);
          setmsg(error);
  ...
  return (
    <div style={{width:'100vw',height:'100vh',backgroundColor:'black',color:'azure',overflowY:'auto'}}>
      ...
      <div className='updatesongdiv2'>
        <h1 style={{fontWeight:'bold'}}>Update the Song</h1>
    ...
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
      ...
      <form >
        <div className='updatesongdiv4'>
        <div className='updatesongdiv5'>
    
 
    </div>
        <div className='updatesongdiv3'>
  <div class="mb-3" style={{width:'70%'}}>
    <label for="exampleInputEmail1" class="form-label">Enter the Song Name</label>
    <input  type="text" placeholder={localStorage.getItem("updatesongname")} onChange={e=>setname(e.target.value)} class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"/>
  </div>
  <div class="mb-3" style={{width:'70%'}}>
    <label for="exampleInputEmail1" class="form-label">Enter Artist Name</label>
    <input type="text" placeholder={localStorage.getItem("updatesongartist")} onChange={e=>setartist(e.target.value)} class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"/>
  </div>
  <div class="mb-3"style={{width:'70%'}}>
  ...
  <div class="mb-3"style={{width:'70%'}}>
    ...
    <input type="text" placeholder={localStorage.getItem("updatesongduration")} onChange={e=>settime(e.target.value)} class="form-control" id="exampleInputPassword1" />
  </div>
  <button type="button" onClick={editsong} className="btn btn-success">Submit</button>
  </div>
